...
if (isfield(initial_dynamics, 'cd')), 
    set_continuous_dynamics(initial_dynamics.cd);
else
    PN.continuous_dynamics = {};
    disp(' Simulating as discrete event system'); disp(' ');
end;

% handle initial colors
if (isfield(initial_dynamics, 'co')),
    set_initial_colors(initial_dynamics.co);
end